...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  void handleMessage(message).then((response) => sendResponse(response)).catch((error) => sendResponse({ ok: false, error: error instanceof Error ? error.message : String(error) }));
  return true;
});

async function handleMessage(message) {
  switch (message.type) {
    case "PLAINLY_GET_SETTINGS": return { ok: true, settings: await getPublicSettings() };
    case "PLAINLY_UPDATE_SETTINGS": await updateSettings(message.settings); return { ok: true, settings: await getPublicSettings() };
    case "PLAINLY_SAVE_API_KEY": await saveApiKey(message.apiKey); return { ok: true, settings: await getPublicSettings() };
    case "PLAINLY_REMOVE_API_KEY": await chrome.storage.local.remove("openAIApiKey"); return { ok: true, settings: await getPublicSettings() };
    case "PLAINLY_SIMPLIFY": return { ok: true, blocks: await simplify(message.payload) };
    default: return { ok: false, error: "Unknown Plainly message" };
  }
}
...
async function restrictSecretStorage() {
  try { await chrome.storage.local.setAccessLevel({ accessLevel: "TRUSTED_CONTEXTS" }); }
  catch (error) { console.warn("Plainly could not restrict extension storage access.", error); }
}
