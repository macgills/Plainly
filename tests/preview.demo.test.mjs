...
test("build a shareable deterministic Plainly preview", async () => {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(artifactsPath, "plainly-wikipedia-preview.png"), fullPage: true });
    ...
    await comparisonPage.screenshot({ path: path.join(artifactsPath, "plainly-before-after-preview.png"), fullPage: true });
...
async function decorateWikipediaPreview(page) {
  await page.addStyleTag({ content: `
    ...
    #mw-content-text p { margin: 0 0 22px; } #mw-content-text figure { display: none; }
  ...
  await page.evaluate((label) => {
    ...
    detail.textContent = `Wikipedia adjusted to ${label} · deterministic demonstration`;
...
function buildComparisonHtml(blocks) {
  const cards = blocks.map((block, index) => `
    <section class="pair">
      ...
      <div class="arrow">→</div>
      <div class="card adjusted"><div class="eyebrow">Plainly · ${TARGET.label}</div><p>${escapeHtml(block.adjusted)}</p></div>
    </section>`).join("");
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Plainly prototype preview</title><style>
    *{box-sizing:border-box}body{margin:0;background:#f5f6f8;color:#17191c;font:18px/1.55 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}main{width:min(1180px,calc(100% - 48px));margin:auto;padding:58px 0 70px}.brand{font-size:15px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}h1{max-width:820px;margin:8px 0 10px;font-size:46px;line-height:1.08}.meta{color:#62676f}.pair{display:grid;grid-template-columns:minmax(0,1fr) 46px minmax(0,1fr);margin:22px 0}.card{min-height:210px;padding:26px 28px;border:1px solid #d8dbe1;border-radius:14px;background:white}.adjusted{border-width:2px}.eyebrow{margin-bottom:16px;font-size:13px;font-weight:800;letter-spacing:.07em;text-transform:uppercase;color:#62676f}.arrow{display:grid;place-items:center;font-size:28px;color:#747981}
  </style></head><body><main><div class="brand">Plainly</div><h1>The same source, at the reader's level.</h1><div class="meta">Deterministic prototype · Photosynthesis · ${TARGET.label}</div>${cards}</main></body></html>`;
}
